...
-(void)dealloc{
    // Ensure notification observer is removed if task is deallocated
    // NSNotificationCenter's removeObserver is thread-safe in macOS 10.11+
    if(_statusHandle){
...
-(NSString*)getDescription{
    if(self.Files){
        if(self.Files.count > 1){
            ...
            return NSLocalizedString(@"reprocessAlbum", @"Process as Album...");
        }
...
-(void)doProcessing:(NSArray<NSString*>*)arguments{
    ...
    __weak NSFileHandle* weakHandle = _statusHandle;
    _task.terminationHandler = ^(NSTask* myself)
    {
        // Create strong reference for observer removal
        Mp3GainTask* strongSelf = weakSelf;
        
        // Remove the notification observer first to prevent dangling pointer issues
        // NSNotificationCenter's removeObserver is thread-safe in macOS 10.11+
        // Use strongSelf because the observer was registered with self, not weakSelf
        if(strongSelf && weakHandle){
            [[NSNotificationCenter defaultCenter] removeObserver:strongSelf name:NSFileHandleDataAvailableNotification object:weakHandle];
        }
